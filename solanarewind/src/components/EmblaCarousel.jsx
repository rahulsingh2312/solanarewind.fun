    ...
    if (publicKey) {
      ...
      if (storedData) {
        ...
        const filteredPoints = analysisPoints.filter(point =>
          /^\d+\.\s\*\*[^*]+/.test(point) // Matches only lines starting with "number. **text"
        );
...
const Slide2 = ({ opacity = 0.5, slideData }) => {
  const [rawTitle, rawDescription] = (slideData?.content || ":").split(/:(.*)/s);
          ...
          <div className="col-12 mt-5 d-flex justify-content-center">
            <div className="box" onMouseOver={() => setHover(true)}>
              <div className="box-body">
                {icon && <img className="img" src={icon} alt={`${symbol} logo`} />}
